# myeloN4.R Model of Myeloid Cell Maturation and trafficking
#
...
#cn = CNDAC = SSB producer = DSBs in S via HR  (5)
...
rcj13 <- function(Time, State, Pars) {
	with(as.list(c(State, Pars)), {
				...
				dBACT   =-vBACT.X
				dabl    =vX.ABL-vABL.X
				dil8    =vIL8.X   # TR: changed sign
				dtnfa   =vTNFA.X  # TR: changed sign
...
X0=c(P=6.466e6, Q=5.655e5, N=4.558e6, TN=7.564e4, AN=76.84,
     ...
     bact=0, abl=0, il8=4, tnfa=0, g=1, il3=1, gsh=1e6, GSSG=0)                                                                            # Starting values for variables
# bact=1e6, abl=0, il8=4, tnfa=0, g=1, il3=1, gsh=1e6, GSSG=0)  # take out bacteria so no perturbation                                                                          # Starting values for variables
       # When bact>0 initialise il8 to bact*5e-4 +4
		   # When bact>0 initialise tnfa to bact*5e-6
		   # When bact>0 initialise abl to tnfa
times <- seq(1,80, by = 1)
out   <- ode(X0, times, rcj13, pars) #,method="lsode")
head(out)
tail(out)  
X0=out[80,2:19]
