...
takes 2D surface fits only up to 2019. To avoid wasting 2020 to 2022 data, a third variant approximates background OC mortality as AC mortality, 
...
Three preliminary steps are needed before running the 2D spline scripts Fig5A_2Dto22.R and Fig5A_2Dto19.R: 1) Fig5A_1mkAges.R, which estimates 
...
fitting; and 3) Fig5A_3mkSpline.R, which fits 2D surface splines  to create single-year resolution mortality matrices.  
Using 2D spline fits that include COVID years 2020 to 2022, Figure 5A is 
![](pngs/Fig5A_2Dto22.png)
Note that at the final time point, excess OC mortality is 1%. To see how much OC excess mortality in this plot 
is due to COVID, Fig5A_2Dto19A.R restricts survival times to end at the end of 2019. Fig. 5A is then
...
The third approach assumes background other cause (OC) mortality approximately equals all cause (AC) mortality, i.e. that 
...
bigger than E, so this approximation is valid.  Generated by Fig5A.R, Figure 5A is then
![](pngs/Fig5A.png)
Advantages of this 3rd approach include using 2020-2022 SEER data and avoiding surface-spline fits. This plot is used in the paper. 
