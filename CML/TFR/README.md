...
mortality matrices from 5-year age bin matrices provided by SEER. The first variant fits 2D surface up through 2022, the most recent year in 
SEER's most recent release. The problem with this is that excess deaths via COVID are then scored as excess deaths by TKI. To avoid this, the second variant
takes 2D surface fits only up to 2019. To avoid wasting 2020 to 2022 data, a third approach approximates OC mortality as AC mortality, 
and HM mortality as zero. The latter two give essentially the same results, which we take to be correct. The third approach is simpler and used in the paper. 


Three preliminary steps are needed before running the 2D spline scripts Fig5_2Dto22.R and Fig5_2Dto19.R. They are: 1) Fig5_1mkAges.R, which estimates 
the midpoint ages of 5-year age intervals; 2) Fig5_2mkMrts.R which prepares 5-year age-interval SEER mortality data for surface spline 
fitting; and 3) Fig5_3mkSpline.R files that perform surface spline fitting to create single-year resolution mortality matrices.  


Using 2D spline fits that include COVID years 2020 to 2022, Figure 5A is 
![](pngs/Fig5_2Dto22.png)
Note that at the final time point, excess OC mortality is 1%.


To see how much other cause (OC) excess mortality in this plot is due to COVID, Fig5_2Dto19.R restricts survival times 
to end at the end of 2019. The new Fig. 5A is then
![](pngs/Fig5_2Dto19.png)
This figure shows that avoiding COVID years reduces excess OC mortality, e.g. now it is below 1% at the final time point. 


A different approach is to assume that other cause (OC) mortality approximately equals all cause (AC) mortality, i.e. that 
for CML patient deaths by HM, EAR=(O-E)/PY is well approximated by O/PY; in the dataframe used to make Figure 5, O is 20-50 fold 
bigger than E, so this approximation is valid.  Generated by fig5_OCequalsAC.R, the new Fig. 5 is then
![](pngs/Fig5_OCequalsAC.png)
Advantages of this approach include using SEER data from 2020, 2021, and 2022 and avoiding surface-spline fits. This is Figure 5A. 
